...
def train_model(model, X, y, epoch, optimizer, criterion, device):
    for i in range(epoch):
        ...
        # print("y dtype:", torch.Tensor(y).dtype)
        loss = criterion(torch.squeeze(outputs),
...
def evaluate(model, X, y, criterion, device):
    ...
    loss = criterion(torch.squeeze(outputs), torch.Tensor(
        y).type(torch.LongTensor).to(device)).item()
    _, predicted = outputs.max(1)
    acc = predicted.eq(torch.Tensor(y).type(
        torch.LongTensor).to(device)).sum().item()/len(y)
    return loss, acc
...
def evaluate_all_loss(model, X, y, device):
    ...
    outputs = model(torch.Tensor(X).to(device))
    # likelihoods = F.softmax(outputs, dim=1).cpu().detach().numpy()
    loss = criterion(torch.squeeze(outputs), torch.Tensor(y).type(
        torch.LongTensor).to(device)).cpu().detach().numpy()
    return loss
...
def perturb_all_weights3(base_model, nn_arch, args, criterion, X, y, Xt, yt, idx_target, nclass, likelihood_tol, loss_percentage, device):
    ...
    for i in range(nclass):
        ...
        optimizer = torch.optim.SGD(
            model.parameters(), lr=args.searchlr, momentum=args.moment)
...
def perturb_all_weights_cv3(base_model, args, criterion, X, y, testloader, idx_target, nclass, likelihood_tol, loss_percentage, device):
    _, test_loss, _ = eval_model_torch(
        testloader, base_model, criterion, device)
    print(test_loss)
    loss_tol = test_loss + loss_percentage
    nfeature = X.shape[1:]

    x_target = X[idx_target].reshape(
        (1, nfeature[0], nfeature[1], nfeature[2]))
    y_target = y[idx_target]
    X_red = np.delete(X, idx_target, axis=0)
    y_red = np.delete(y, idx_target)
    ...
    for i in range(nclass):
        ...
        print(' Build time: {:.2f} mins'.format(
            (time.mktime(time.localtime()) - time.mktime(build_time))/60))
        ...
        optimizer = torch.optim.SGD(
            model.parameters(), lr=args.searchlr, momentum=args.moment)
        print(' Clone time: {:.2f} mins'.format(
            (time.mktime(time.localtime()) - time.mktime(clone_time))/60))
...
def train_model_torch(log, loader, args, nclass, criterion, device):
    ...
    model = model.to(device)
    optimizer = optim.SGD(model.parameters(), lr=args.trainlr,
                          momentum=0.9, weight_decay=5e-4)

    for epoch in range(args.nepoch):
        model.train()  # prep model for training
        for batch_i, (inputs, targets) in enumerate(loader, start=0):
            inputs, targets = inputs.to(device), targets.type(
